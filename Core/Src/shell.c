...
char *shell_execute_mut(char *input_mut, int input_size)
{
  ...
  for (int i = 0; i < sizeof(commands_map) / sizeof(commands_map[0]); ++i)
  {
    if (strcmp(commands_map[i].cmd, tokens[0]) == 0)
    {
      return commands_map[i].func(tokens, sizeof(tokens) / sizeof(tokens[0]));
    }
  }
...
static char *start_leds(char **tokens, int max_tokens)
{
  return control_leds(tokens, max_tokens, GPIO_PIN_SET);
}

static char *stop_leds(char **tokens, int max_tokens)
{
  return control_leds(tokens, max_tokens, GPIO_PIN_RESET);
}
